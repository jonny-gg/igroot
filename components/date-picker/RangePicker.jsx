...
export default class RangePicker extends React.Component {
    constructor(props) {
        ...
        this.renderFooter = () => {
            ...
            const operations = Object.keys(ranges).map((range) => {
                ...
                return (<a key={range} onClick={() => this.setValue(value)} onMouseEnter={() => this.setState({ hoverValue: value })} onMouseLeave={this.clearHoverValue}>
          {range}
        </a>);
            });
            return (<div className={`${prefixCls}-range-quick-selector`}>
        {operations}
      </div>);
        };
    ...
    render() {
        ...
        const input = ({ value: inputValue }) => {
            ...
            return (<span className={props.pickerInputClass} disabled={props.disabled}>
          <input disabled={props.disabled} readOnly={true} value={(start && start.format(props.format)) || ''} placeholder={startPlaceholder} className={`${prefixCls}-range-picker-input`}/>
          <span className={`${prefixCls}-range-picker-separator`}> ~ </span>
          <input disabled={props.disabled} readOnly={true} value={(end && end.format(props.format)) || ''} placeholder={endPlaceholder} className={`${prefixCls}-range-picker-input`}/>
          {clearIcon}
          <span className={`${prefixCls}-picker-icon`}/>
        </span>);
        };
        return (<span className={props.pickerClass} style={assign({}, style, pickerStyle)}>
