...
import { Row, Col, Menu, Icon, Tabs } from 'igroot';
import classNames from 'classnames';
import Article from '../Content/Article';
import ComponentDoc from '../Content/ComponentDoc';
...
function getModuleData(props) {
    ...
    const moduleName = /^\/?bcomponents/.test(pathname) ?
        'bcomponents' : pathname.split('/').filter(item => item).slice(0, 2).join('/');
    const moduleData = moduleName === 'bcomponents' || moduleName === 'docs/business' ||
        moduleName === 'changelog' || moduleName === 'changelog-cn' ?
        [...props.picked.bcomponents, ...props.picked['docs/business'], ...props.picked.changelog] :
...
export default class CompsContent extends React.Component {
    static contextTypes = {
    ...
    getSideBarOpenKeys(nextProps) {
        ...
        this.currentModule = pathname.replace(/^\//).split('/')[1] || 'bcomponents';
        if (this.currentModule === 'business') {
            this.currentModule = 'bcomponents';
        }
    ...
    generateMenuItem(isTop, item) {
        ...
        const child = !item.link ? (
            <Link
                to={utils.getLocalizedPathname(/^bcomponents/.test(url) ? `${url}/` : url, locale === 'zh-CN')}
                disabled={disabled}
            >
                {text}
            </Link>
